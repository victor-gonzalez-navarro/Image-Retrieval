function algo3MADmej(I,bins)

fileInput = fopen('input2.txt', 'r');
fileOutput = fopen('output.txt', 'w');
numimagesinputtxt = 2000;
nom = 'ukbench';
extensio = '.jpg';
guardat1 = zeros(10,1);
guardat2 = zeros(10,1);
...
for j=1:numimagesinputtxt
        ...
        h = imhist(rgb2gray(imread(A)));

        for i=1:2000
            M1 = algo2versio1(I(i,:), h,bins);
            distance1(i) = M1;
            
            M2 = algo2versio2(I(i,:), h,bins);
            distance2(i) = M2;
            
            M3 = algo2versio3(I(i,:), h,bins);
            distance3(i) = M3;
            
            M4 = algo2versio4(I(i,:), h,bins);
            distance4(i) = M4;
        end
        ...
        limits = [0 1 0 1];
        figure(50), plot(guardat2, guardat1, 'r-*', guardat4, guardat3, 'y-*', guardat6, guardat5, 'm-*', guardat8, guardat7, 'b-*'), xlabel('Recall'), ylabel('Precision'), title('Precision/Recall 2000 images'),
